/**
 * Kinema Rendering Engine - Performance Statistics
 *
...
export class RenderStatsCollector {
  ...
  endFrame(): void {
    ...
    if (frameEndTime - this.fpsUpdateTime >= 500) {
      this.stats.fps = Math.round(
        (this.framesSinceUpdate * 1000) / (frameEndTime - this.fpsUpdateTime),
      );
   ...
   * Record a draw call
   */
  recordDrawCall(_instanceCount: number = 1, isInstanced: boolean = false): void {
   ...
   */
  beginRenderPass(_label: string): void {
