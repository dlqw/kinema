/**
 * Kinema Rendering Engine - Render Context
 *
...
export class RenderContextImpl implements RenderContext {
  ...
  public readonly presentationFormat: TextureFormat;
  ...
  private constructor(
    ...
    this.device = device;
    this.config = config;
    this.presentationFormat = device.presentationFormat as TextureFormat;
   ...
   * ```
   */
  static async create(config: RenderEngineConfig): Promise<RenderContextImpl> {
    ...
    const deviceConfig: GraphicsDeviceConfig = {
      ...
      apiPreference: config.apiPreference ?? RenderAPI.WebGPU,
    ...
    const context = new RenderContextImpl(canvas, device, config);
  ...
  private updateSize(): void {
    ...
    if (clientWidth > 0 && clientHeight > 0) {
      ...
      if (
        newWidth !== this.device.presentationSize[0] ||
        newHeight !== this.device.presentationSize[1]
      ) {
        // Update device
        this.device.resize();
