/**
 * Kinema Rendering Engine - Path Renderer (Canvas 2D)
 *
...
export class PathRenderer {
  ...
  stroke(color: string, lineWidth?: number): void {
    const options: PathOptions = { stroke: color };
    if (lineWidth !== undefined) {
      options.lineWidth = lineWidth;
    }
    this.draw(this.currentPath, options);
  }
  ...
  drawPolygon(points: [number, number][], options: PathOptions = {}): void {
    ...
    if (firstPoint !== undefined) {
      commands.push({ type: 'move', x: firstPoint[0], y: firstPoint[1] });

      // Line to remaining points
      for (let i = 1; i < points.length; i++) {
        const point = points[i];
        if (point !== undefined) {
          commands.push({ type: 'line', x: point[0], y: point[1] });
        }
      }
   ...
   * @param options - Rendering options
   */
  drawCurve(points: [number, number][], tension = 0.5, options: PathOptions = {}): void {
    if (points.length < 2) {
      return;
    }
    ...
    // Move to first point - we know points[0] exists since length >= 2
    const firstPoint = points[0]!;
    commands.push({ type: 'move', x: firstPoint[0], y: firstPoint[1] });
    ...
    for (let i = 0; i < points.length - 1; i++) {
      ...
      // All these points are guaranteed to exist due to the bounds we're using
      // and the fact that points.length >= 2
      if (p0 && p1 && p2 && p3) {
  ...
  drawArrow(
    ...
    const arrowOptions: PathOptions = { ...options };
    delete (arrowOptions as Partial<PathOptions>).fill;
    this.draw(commands, arrowOptions);
  }
}
