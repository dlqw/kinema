/**
 * Kinema Rendering Engine - Circle Renderer (Canvas 2D)
 *
...
export class CircleRenderer {
  ...
  stroke(x: number, y: number, radius: number, color: string, lineWidth?: number): void {
    const options: CircleOptions = { stroke: color };
    if (lineWidth !== undefined) {
      options.lineWidth = lineWidth;
    }
    this.draw(x, y, radius, options);
  }
   ...
   */
  drawGradient(x: number, y: number, radius: number, innerColor: string, outerColor: string): void {
    const ctx = this.renderer.getContext();
