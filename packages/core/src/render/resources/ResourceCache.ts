/**
 * Kinema Rendering Engine - Resource Cache
 *
...
export class ResourceCache<T = GraphicsBuffer | GraphicsTexture | GraphicsSampler> {
  ...
  private ensureCapacity(requiredSize: number): void {
    ...
    if (this.config.enableMemoryTracking && maxSize > 0) {
      while (this.currentMemoryUsage + requiredSize > maxSize && this.accessOrder.length > 0) {
        this.evictOne();
      }
    }
  }
...
// Types and enums are already exported inline
