/**
 * Kinema Rendering Engine - Shader Manager
 *
...
import type { GraphicsShader, ShaderModuleDescriptor } from '../core/types';
...
export class ShaderManager {
  ...
  // Reserved for future file watching feature
  // @ts-ignore
  private _fileWatcherMap: Map<string, unknown> = new Map();
  ...
  async loadShader(descriptor: ShaderDescriptor): Promise<ShaderCompilationResult> {
    ...
    const processedSource = this.processShaderSource(descriptor.code, descriptor.defines || {});
    ...
    try {
      ...
      warnings = compilationInfo.messages.filter((m) => m.type === 'warning').map((m) => m.message);
   ...
   * @returns Promise resolving to compilation result
   */
  async loadShaderFromFile(filePath: string, label?: string): Promise<ShaderCompilationResult> {
  ...
  removeShader(label: string): boolean {
    const entry = this.shaderLibrary.get(label);
    if (entry) {
      if ('destroy' in entry.shader && typeof entry.shader.destroy === 'function') {
   ...
   */
  clear(): void {
    for (const [_label, entry] of this.shaderLibrary) {
      if ('destroy' in entry.shader && typeof entry.shader.destroy === 'function') {
        entry.shader.destroy();
      }
    }
  ...
  private cacheShader(
    ...
    this.shaderLibrary.set(descriptor.label || 'unnamed', {
      ...
      result,
      refCount: 1,
      lastModified: descriptor.filePath ? Date.now() : 0,
    });
  }
