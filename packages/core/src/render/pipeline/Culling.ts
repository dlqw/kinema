/**
 * Kinema Rendering Engine - Frustum Culling
 *
...
export class FrustumCuller {
  ...
  private hashCamera(camera: Camera): number {
    ...
    return Math.floor(pos[0] * 100) ^ Math.floor(pos[1] * 100) ^ Math.floor(pos[2] * 100);
  }
}
...
export class OcclusionCuller {
  private _hardwareOcclusionSupported: boolean = false;

  constructor() {
    // Detect hardware occlusion query support
    this._hardwareOcclusionSupported = this.detectOcclusionSupport();
  }

  /**
   * Check if hardware occlusion queries are supported
   * @returns True if hardware occlusion is available
   */
  get isHardwareSupported(): boolean {
    return this._hardwareOcclusionSupported;
  }
   ...
   *
   * @param objects - Objects to test
   * @param _camera - Camera (reserved for future use)
   * @returns Visible objects
   */
  cullSoftware(objects: RenderableObject[], _camera: Camera): RenderableObject[] {
    ...
    for (const obj of sorted) {
      ...
      if (this.isLargeObject(obj)) {
        occluders.push(this.boundsFromSphere(obj.bounds));
      }
    }
   ...
   *
   * @param _bounds - Bounding sphere (reserved for future use)
   * @param _occluders - List of occluder bounding boxes (reserved for future use)
   * @returns True if occluded
   */
  private isOccluded(_bounds: BoundingSphere, _occluders: BoundingBox[]): boolean {
