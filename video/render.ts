 ...
 * Video Renderer
 *
 * Renders Kinema scenes to video files using Canvas and FFmpeg
 */
...
export class VideoRenderer {
  ...
  async render(config: VideoRenderConfig): Promise<string> {
    ...
    for (let sceneIndex = 0; sceneIndex < this.scenes.length; sceneIndex++) {
      ...
      console.log(
        `📹 Rendering scene ${sceneIndex + 1}/${this.scenes.length} (${scene.duration}s, ${sceneFrames} frames)`,
      );
      ...
      for (let i = 0; i < sceneFrames; i++) {
        ...
        if (frameIndex % 60 === 0) {
          console.log(
            `   Progress: ${((frameIndex / totalFrames) * 100).toFixed(1)}% (${frameIndex}/${totalFrames})`,
          );
        }
      }
    }
