...
function createTypeInferenceDemo(scene: Scene, startTime: number) {
  ...
  ;[codePanel.background, codePanel.titleText, intellisenseBox,
    ...suggestionTexts, ...codeTexts, cursor, subtitle1, subtitle2].forEach((el) => {
    scene.timeline.add({
      target: el,
      startTime: startTime + 18,
      duration: 1,
...
function createErrorCatchingDemo(scene: Scene, startTime: number) {
  ...
  // 清理
  const allElements = [
    errorPanel.background, errorPanel.titleText,
    correctPanel.background, correctPanel.titleText,
    errorTooltip, errorTooltipText, successCheck,
    subtitle1, subtitle2,
    ...errorTexts, ...correctTexts,
  ]
...
function createTypeSafetyMatrix(scene: Scene, startTime: number) {
  ...
  // 四象限标题
  const quadrants = [
    { title: '🔍 参数校验', x: centerX - 250, y: centerY - 100, items: ['必填/可选', '类型匹配', '范围检查'] },
    { title: '🎯 返回值类型', x: centerX + 250, y: centerY - 100, items: ['自动推导', '链式调用保证', '空值安全'] },
    { title: '🏗️ 泛型约束', x: centerX - 250, y: centerY + 100, items: ['动画目标类型', '状态类型关联', '插值结果类型'] },
    { title: '📦 品牌类型', x: centerX + 250, y: centerY + 100, items: ['ObjectId', 'Time / Alpha', '编译时区分'] },
  ]
