...
本指南帮助你从 GSAP (GreenSock Animation Platform) 迁移到 AniMaker。
...
import { easeOut } from '@animaker/core/easing';

const scene = createScene();
const box = VectorObject.rectangle(1, 1);

const moveX = new MoveAnimation(box, { x: 2, y: 0, z: 0 }, {
  duration: 1,
  easing: easeOut
});

const moveY = new MoveAnimation(box, { x: 2, y: 1, z: 0 }, {
  duration: 1,
  easing: easeOut
});
...
```typescript
// AniMaker
const transform = new TransformAnimation(box, {
  id: box.getState().id,
  transform: {
...
import { easeOut, easeInOut, elastic } from '@animaker/core/easing';
...
const moveY = new MoveAnimation(box, { x: 2, y: 0.5, z: 0 }, {
  duration: 0.5,
  easing: easeInOut
});
...
const animations = boxes.map(box =>
  new MoveAnimation(box, { x: box.getState().transform.position.x + 1, y: 0, z: 0 }, {
    duration: 1,
    easing: easeOut
  })
);

const staggered = new AnimationGroup(
  boxes[0],
  animations,
...
function createGridStagger(
  ...
  // 按距离中心排序
  const sorted = objects.map((obj, index) => ({
    obj,
    index,
...
// 使用
const gridAnimations = createGridStagger(boxes, box =>
  new MoveAnimation(box, { x: box.getState().transform.position.x + 1, y: 0, z: 0 }, {
    duration: 1,
    easing: easeOut
  })
...
const gridStaggered = new AnimationGroup(
  boxes[0],
  gridAnimations,
  CompositionType.Lagged
);
...
const moveX = new MoveAnimation(box, { x: 5, y: 0, z: 0 }, {
  duration: 1,
...
// 使用
const box = VectorObject.rectangle(1, 1).withPosition({ x: 0, y: 0, z: 0 });
const draggable = new DraggableObject(box, {
  min: { x: -2, y: -2, z: 0 },
  max: { x: 2, y: 2, z: 0 }
}, true);
