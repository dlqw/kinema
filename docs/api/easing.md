...
本节介绍 Kinema 框架的缓动函数（Easing Functions）API。缓动函数控制动画的速度曲线，是创建自然、流畅动画的关键。
...
```typescript
import { linear } from '@kinema/core/easing';
...
```typescript
const popIn = new ScaleAnimation(
  obj,
  { x: 1.5, y: 1.5, z: 1 },
  {
    duration: 1,
...
import { smooth, easeInOut, easeOutBack, elastic, bounce } from '@kinema/core/easing';
...
// 3. 弹性移动
const elasticMove = new MoveAnimation(
  obj,
  { x: 2, y: 0, z: 0 },
  {
    duration: 1.5,
