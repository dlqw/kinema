...
本节介绍 AniMaker 框架的动画系统 API。动画系统是框架的核心，负责定义和执行对象属性随时间的变化。
...
import { easeInSine } from '@animaker/core/easing';
...
```typescript
import { RotateAnimation } from '@animaker/core/animation';
import { smooth } from '@animaker/core/easing';
...
// 对角线移动
const moveDiagonal = new MoveAnimation(circle, {
  x: 2,
  y: 2,
  z: 0
}, {
...
```typescript
import { ScaleAnimation } from '@animaker/core/animation';
import { easeOutBack } from '@animaker/core/easing';

// 放大到 1.5 倍
const scaleUp = new ScaleAnimation(circle, {
  x: 1.5,
  y: 1.5,
  z: 1
}, {
...
// 仅水平拉伸
const stretchHorizontal = new ScaleAnimation(rect, {
  x: 2,
  y: 1,
  z: 1
}, {
...
import { MoveAnimation, RotateAnimation } from '@animaker/core/animation';

// 同时移动和旋转（像车轮滚动）
const parallelGroup = new AnimationGroup(
  wheel,
  [
    new MoveAnimation(wheel, { x: 3, y: 0, z: 0 }, {
      duration: 2,
      easing: smooth
    }),
...
const laggedGroup = new AnimationGroup(
  circles[0],
  circles.map(circle =>
    new MoveAnimation(circle, { x: 0, y: 2, z: 0 }, {
      duration: 1,
      easing: smooth
    })
  ),
...
```typescript
import { FadeInAnimation, RotateAnimation } from '@animaker/core/animation';
import { smooth, easeOutBack } from '@animaker/core/easing';
