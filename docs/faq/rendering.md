...
AniMaker 支持 WebGPU 和 Canvas2D 两种渲染器，应该如何选择？
...
// 转换 Canvas 坐标到 AniMaker 坐标
function canvasToAniMaker(x: number, y: number, width: number, height: number): Point3D {
  return {
...
// 自定义透明度动画
const customFade = new TransformAnimation(circle, {
...
const obj = VectorObject.circle(1)
  .withStyles(new Map([
    ['fillColor', convertToSRGB('#3498db')]
  ]));
```
