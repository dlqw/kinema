/**
 * AniMaker Rendering Engine - Buffer Manager
 *
...
import type { BufferDescriptor, BufferUsage } from '../core/types';
...
export class BufferManager {
   ...
   * @returns Buffer handle
   */
  createBuffer(descriptor: BufferDescriptor & BufferOptions): BufferHandle {
    ...
    if (!buffer) {
      // Create new buffer
      const gpuDescriptor: BufferDescriptor = {
    ...
    this.log(
      `[BufferManager] Created buffer: ${bufferId} (${this.formatSize(descriptor.size)})`
    );
   ...
   */
  private addToPool(pool: Map<number, GraphicsBuffer[]>, size: number, buffer: GraphicsBuffer): void {
