/**
 * AniMaker Rendering Engine - Texture Manager
 *
...
export class TextureManager {
  ...
  async createTexture2D(descriptor: Texture2DDescriptor): Promise<TextureHandle> {
    ...
    const mipLevelCount =
      descriptor.mipLevelCount ?? (descriptor.generateMipmaps ? this.calculateMipLevels(descriptor.width, descriptor.height) : 1);
    ...
    if (descriptor.persistent) {
      this.textureCache.set(
        ...
        this.calculateTextureSize(descriptor.width, descriptor.height, textureDescriptor.format, mipLevelCount),
        true
      );
    }
  ...
  async uploadTextureData(
    ...
    if (descriptor.data instanceof ArrayBufferView) {
  ...
  destroyAll(): void {
    ...
    for (const [id, handle] of this.samplers) {
