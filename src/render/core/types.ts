/**
 * AniMaker Rendering Engine - Core Type Definitions
 *
...
export type BufferUsage = 1 | 2 | 3 | 4 | 5 | 6 | 7; // MAP_READ | MAP_WRITE | COPY_SRC | COPY_DST | INDEX | VERTEX | UNIFORM | STORAGE | INDIRECT
...
export interface CommandEncoder {
  ...
  copyBufferToBuffer(source: GraphicsBuffer, sourceOffset: number, destination: GraphicsBuffer, destinationOffset: number, size: number): void;
  copyBufferToTexture(source: ImageCopyBuffer, destination: ImageCopyTexture, copySize: [number, number] | [number, number, number]): void;
  copyTextureToBuffer(source: ImageCopyTexture, destination: ImageCopyBuffer, copySize: [number, number] | [number, number, number]): void;
  copyTextureToTexture(source: ImageCopyTexture, destination: ImageCopyTexture, copySize: [number, number] | [number, number, number]): void;
...
export interface RenderPassEncoder {
  ...
  setIndexBuffer(buffer: GraphicsBuffer, indexFormat: IndexFormat, offset?: number, size?: number): void;
...
export interface Queue {
  ...
  writeBuffer(buffer: GraphicsBuffer, bufferOffset: number, data: ArrayBufferView, dataOffset?: number, size?: number): void;
  writeTexture(destination: ImageCopyTexture, data: ArrayBufferView, dataLayout: ImageDataLayout, size: [number, number] | [number, number, number]): void;
  copyExternalImageToTexture(source: ImageCopyExternalImage, destination: ImageCopyTexture, copySize: [number, number] | [number, number, number]): void;
}
