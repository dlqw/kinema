/**
 * AniMaker Rendering Engine - Canvas Context Manager
 *
...
import type { RenderCapability } from '../core/Capability';
import type { GraphicsDevice } from '../graphics/GraphicsDevice';
import { CanvasRenderer } from './CanvasRenderer';
import { RenderAPI } from '../core/Capability';
 ...
 *
 * Canvas 2D rendering context that provides compatibility
 * with the GPU rendering context interface.
 *
 * @example
 * ```typescript
 ...
 * ```
 */
export class CanvasContextImpl implements RenderContext {
  public readonly canvas: HTMLCanvasElement;
  public readonly device: CanvasRenderer;
  public readonly presentationSize: [number, number];
  public readonly aspect: number;
  public readonly presentationFormat: GPUTextureFormat = 'bgra8unorm';

  private config: CanvasContextConfig;
  ...
  static async create(
    ...
    const context = new CanvasContextImpl(
      config.canvas,
      renderer,
      config,
      capability
  ...
  getInfo(): Record<string, any> {
    return {
      ...
      devicePixelRatio: this.device.getPixelRatio(),
    };
  }
}
...
// Re-export types
export type { CanvasContextConfig };
