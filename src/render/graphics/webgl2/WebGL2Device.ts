/**
 * AniMaker Rendering Engine - WebGL2 Device Implementation
 *
...
export class WebGL2Device implements GraphicsDevice {
  ...
  createSampler(descriptor: SamplerDescriptor): GraphicsSampler {
    ...
    gl.samplerParameteri(sampler, gl.TEXTURE_MIN_FILTER, this.mapFilterMode(descriptor.minFilter, descriptor.mipmapFilter));
    ...
    if (descriptor.maxAnisotropy !== undefined) {
      ...
      if (ext) {
        gl.samplerParameterf(sampler, (ext as any).TEXTURE_MAX_ANISOTROPY_EXT, descriptor.maxAnisotropy);
      }
    }
    if (descriptor.compare !== undefined) {
      const ext = gl.getExtension('EXT_shadow_samplers');
      if (ext) {
        gl.samplerParameteri(sampler, (ext as any).TEXTURE_COMPARE_FUNC, this.mapCompareFunction(descriptor.compare));
      }
    }
  ...
  private mapLimits(gl: WebGL2RenderingContext): DeviceLimits {
    ...
    return {
      ...
      maxVertexBuffers: getLimit(gl.MAX_VERTEX_ATTRIB_BINDINGS),
      maxVertexAttributes: getLimit(gl.MAX_VERTEX_ATTRIBS),
      maxVertexBufferArrayStride: 2048, // Reasonable default
    };
  }

  private createAdapterInfo(gl: WebGL2RenderingContext): GPUAdapterInfo {
...
class WebGL2Buffer implements GraphicsBuffer {
  ...
  constructor(gl: WebGL2RenderingContext, buffer: WebGLBuffer, descriptor: BufferDescriptor, resourceId: number, target: GLenum) {
    this._gl = gl;
    this._buffer = buffer;
...
class WebGL2Texture implements GraphicsTexture {
  ...
  constructor(gl: WebGL2RenderingContext, texture: WebGLTexture, descriptor: TextureDescriptor, resourceId: number) {
    this._gl = gl;
...
class WebGL2Shader implements GraphicsShader {
  ...
  private _descriptor: ShaderModuleDescriptor;
...
class WebGL2Query implements any {
  private _gl: WebGL2RenderingContext;
...
class WebGL2CommandEncoder implements CommandEncoder {
  ...
  private _gl: WebGL2RenderingContext;
  private _device: WebGL2Device;
  private _commands: Array<() => void> = [];

  constructor(gl: WebGL2RenderingContext, device: WebGL2Device) {
    this._gl = gl;
    this._device = device;
  }
  ...
  copyBufferToBuffer(
    ...
    this._commands.push(() => {
      ...
      const srcTarget = (source as WebGL2Buffer).target;
      const dstTarget = (destination as WebGL2Buffer).target;
...
class WebGL2RenderPassEncoder implements any {
  private _gl: WebGL2RenderingContext;
  private _commands: Array<() => void>;
  ...
  setIndexBuffer(buffer: GraphicsBuffer, indexFormat: GPUIndexFormat, offset?: number, size?: number): void {
    this._commands.push(() => {
      const gl = this._gl;
  ...
  drawIndirect(indirectBuffer: GraphicsBuffer, indirectOffset: number): void {
    this._commands.push(() => {
      ...
      gl.bindBuffer(gl.DRAW_INDIRECT_BUFFER, webglBuffer);
      gl.drawArraysIndirect(gl.TRIANGLES, indirectOffset);
    });
  }
  ...
  drawIndexedIndirect(indirectBuffer: GraphicsBuffer, indirectOffset: number): void {
    this._commands.push(() => {
      ...
      const webglBuffer = (indirectBuffer as WebGL2Buffer).nativeBuffer;
      gl.bindBuffer(gl.DRAW_INDIRECT_BUFFER, webglBuffer);
      gl.drawElementsIndirect(gl.TRIANGLES, gl.UNSIGNED_SHORT, indirectOffset);
    });
  }

  insertDebugMarker(markerLabel: string): void {
    if (this._gl.debugMarkerInsert) {
      this._commands.push(() => {
        (this._gl.debugMarkerInsert as any)(this._gl, markerLabel);
      });
    }
  }

  popDebugGroup(): void {
    if (this._gl.debugMarkerPop) {
      this._commands.push(() => {
        (this._gl.debugMarkerPop as any)(this._gl);
      });
    }
  }

  pushDebugGroup(groupLabel: string): void {
    if (this._gl.debugMarkerPush) {
      this._commands.push(() => {
        (this._gl.debugMarkerPush as any)(this._gl, groupLabel);
      });
    }
  }
...
class WebGL2CommandBuffer implements any {
