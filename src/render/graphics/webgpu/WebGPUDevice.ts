/**
 * AniMaker Rendering Engine - WebGPU Device Implementation
 *
...
export class WebGPUDevice implements GraphicsDevice {
  ...
  pushErrorScope(groupLabel: string): void {
    this.checkNotDestroyed();
    this.device.pushErrorScope(groupLabel);
  }
  ...
  private setupErrorHandling(): void {
    ...
    this.device.uncapturederror = (event: GPUUncapturedErrorEvent) => {
...
class WebGPUBuffer implements GraphicsBuffer {
  ...
  write(data: ArrayBufferView, offset: number = 0): void {
    this.checkNotDestroyed();
    this._device.queue.writeBuffer(this._buffer, offset, data.buffer, data.byteOffset, data.byteLength);
  }
...
class WebGPUTexture implements GraphicsTexture {
  ...
  constructor(device: GPUDevice, texture: GPUTexture, descriptor: TextureDescriptor) {
    this._device = device;
...
class WebGPUCommandEncoder implements CommandEncoder {
  ...
  constructor(device: GPUDevice, encoder: GPUCommandEncoder) {
    this._device = device;
...
class WebGPURenderPassEncoder implements any {
  ...
  drawIndexedIndirect(indirectBuffer: GraphicsBuffer, indirectOffset: number): void {
    this._encoder.drawIndexedIndirect((indirectBuffer as WebGPUBuffer).nativeBuffer, indirectOffset);
  }
...
class WebGPUComputePassEncoder implements any {
...
class WebGPUCommandBuffer implements any {
  readonly label: string;
...
class WebGPUQueue implements Queue {
  ...
  writeTexture(destination: any, data: ArrayBufferView, dataLayout: any, size: any): void {
    this._queue.writeTexture(destination, data, dataLayout, size);
  }
