/**
 * AniMaker Rendering Engine - Rendering Sorting
 *
...
export class RenderSorter {
  ...
  private isTransparent(obj: RenderableObject): boolean {
    ...
    if (obj.userData?.transparent !== undefined) {
      return obj.userData.transparent;
    }
  ...
  calculateStateChanges(objects: RenderableObject[]): {
    ...
    for (let i = 1; i < objects.length; i++) {
      if (objects[i].materialId !== lastMaterial) {
        materialChanges++;
        lastMaterial = objects[i].materialId;
      }
      if (objects[i].meshId !== lastMesh) {
        meshChanges++;
        lastMesh = objects[i].meshId;
      }
    }
